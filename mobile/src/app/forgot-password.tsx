...
export default function ForgotPasswordScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.replace({ pathname: '/reset-password', params: { email: trimmed } });
    } catch {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.info}>Nous vous enverrons un code à 6 chiffres par email.</Text>
      ...
      <Pressable style={styles.button} onPress={handleSubmit} disabled={loading}>
        {loading ? <ActivityIndicator color="white" /> : <Text style={styles.buttonLabel}>Envoyer le code</Text>}
      </Pressable>
    </SafeAreaView>
  );
}
