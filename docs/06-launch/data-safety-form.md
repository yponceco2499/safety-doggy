...
- **Proposez-vous un moyen de demander la suppression des données ?** → **Oui**, en libre-service depuis l'app (Profil > Supprimer mon compte). Fonction serveur `delete-account` déployée et vérifiée le 2026-08-22 (répond correctement à un appel de test). Suppression réelle d'un compte pas encore testée de bout en bout — à faire avant de considérer ce point définitivement clos.
